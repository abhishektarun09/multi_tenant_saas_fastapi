    ...
    with open(file_name, "r") as f:
        for line in f.readlines():
            ...
            if line_data["type"] == "Point":
                ...
                metric_data.append(
                    (
                        line_data["data"]["value"],
                        parser.parse(line_data["data"]["time"]),
                    )
                )
...
def get_metric_data(data, watched_metric):
    ...
    return (
        [time for (value, time) in metric_data],
        [value for (value, time) in metric_data],
    )
...
def get_avg_and_max_from_data(metric_data_raw):
    ...
    # compute data avg
    metric_data_avg = [
        val / count for (_, (val, count)) in metric_data_sum_count.items()
    ]
...
def display_chart(metric_data_avg, metric_data_max, vus_data, watched_metric):
    ...
    ax1.plot(
        metric_data_max.keys(),
        metric_data_avg,
        color="blue",
        label=f"avg {watched_metric}",
    )

    # plot max metric data values
    ax1.plot(
        metric_data_max.keys(),
        metric_data_max.values(),
        color="red",
        linestyle="dashed",
        label=f"max {watched_metric}",
    )
    ...
    ax2.plot(vus_data[0], vus_data[1], color="green", label="VU count")
