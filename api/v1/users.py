...
async def register_user(request: Request, user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    existing_user = (await db.execute(select(Users).where(Users.email == user.email))).scalars().first()
    if existing_user:
        raise HTTPException(
...
async def me(db: AsyncSession = Depends(get_db), current_user_and_membership = Depends(get_user_and_membership)):
    
    current_user, membership = current_user_and_membership
    
    user_details = (await db.execute(select(Users).where(Users.id == current_user.id))).scalars().first()
    
    return {"email": user_details.email, "name": user_details.name, "role" : membership.role}

@router.get("/orgs", response_model=ListOrgs)
async def list_orgs(db: AsyncSession = Depends(get_db), current_user: int = Depends(get_current_user)):
    
    user = (await db.execute(select(Users).where(Users.id == current_user.id))).scalars().first()
    
    # List out the organizations the user is part of for frontend to select
    org_ids = (await db.execute(select(OrganizationMember.organization_id).where(OrganizationMember.user_id == user.id))).scalars().all()
...
async def update_password(response: Response, request: Request, input_data: UpdatePasswordIn, current_user: Users = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
